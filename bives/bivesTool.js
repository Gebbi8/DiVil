//Info-Content
function getBivesData(v1, v2, urlFlag, file1, file2) {
	...
	//check if files are on url or locally
	if (urlFlag) {
		commands = ["reportHtml", "reactionsSbgnJson", "xmlDiff", "separateAnnotations"];
		compareModels(v1, v2, commands, urlFlag, file1, file2);
...
function compareModels(f1, f2, command, urlFlag, file1, file2) {
	var bivesJob = {
		files:
			[
				f1,
	...
	$.post(
		...
		function (data) {
			...
			if (!urlFlag) showSbgn(sbgnJson, xmlDiff, comodiAnnotation, f1, f2);
			else showSbgn(sbgnJson, xmlDiff, comodiAnnotation, file1, file2);
